...
export default function ProfilePharmacy({ onSwitchToCustomer }) {
  ...
  const latestFormRef = useRef(verificationForm);
  const latestStepRef = useRef(verificationStep);
  const draftDocIdRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (docSnap) {
          draftDocIdRef.current = docSnap.id;
          setVerificationRequest({ id: docSnap.id, ...docSnap.data() });
        } else {
  ...
  useEffect(() => {
    latestFormRef.current = verificationForm;
  }, [verificationForm]);

  useEffect(() => {
    latestStepRef.current = verificationStep;
  }, [verificationStep]);
  ...
  useEffect(() => {
    ...
    const initialForm = buildVerificationFormState();
    setVerificationForm(initialForm);
    latestFormRef.current = initialForm;
    setVerificationError('');
    const savedStep =
      typeof verificationRequest?.draftStep === 'number'
        ? Math.min(
            Math.max(0, verificationRequest.draftStep),
            totalVerificationSteps - 1
          )
        : 0;
    setVerificationStep(savedStep);
    latestStepRef.current = savedStep;
  ...
  const buildVerificationPayload = (form) => {
    const documents = [
      form.licenseUrl?.trim()
        ? {
            name: 'Pharmacy License',
            url: form.licenseUrl.trim(),
          }
        : null,
      form.ownerIdUrl?.trim()
        ? {
            name: 'Owner Identification',
            url: form.ownerIdUrl.trim(),
          }
        : null,
    ].filter(Boolean);
    const location = {
    ...
    return {
      entityName: form.pharmacyName?.trim() || '',
  ...
  const runVerificationAutoSave = async (
    formOverride,
    stepOverride
  ) => {
    if (!user?.uid || verificationSubmitting) return;
    autoSaveTimeoutRef.current = null;
    if (!verificationModalOpen) return;
    const form = formOverride || latestFormRef.current;
    const step =
      typeof stepOverride === 'number'
        ? stepOverride
        : latestStepRef.current;
    const baseUpdate = buildVerificationPayload(form);
    const payload = {
      ...baseUpdate,
      draftStep: step,
    ...
    try {
      ...
      const targetDocId =
        verificationRequest?.id || draftDocIdRef.current;
      if (targetDocId) {
        await updateDoc(doc(db, 'verificationQueue', targetDocId), {
          ...payload,
        });
      } else {
        const newDoc = await addDoc(collection(db, 'verificationQueue'), {
          ...payload,
        ...
        draftDocIdRef.current = newDoc.id;
      }
    } catch (error) {
      console.error('[ProfilePharmacy] verification autosave failed', error);
    } finally {
      setIsAutoSaving(false);
    }
  };
  const queueVerificationAutoSave = (formOverride, stepOverride) => {
    if (!verificationModalOpen || verificationSubmitting) return;
    if (autoSaveTimeoutRef.current) {
      clearTimeout(autoSaveTimeoutRef.current);
    }
    const form = formOverride || latestFormRef.current;
    ...
    autoSaveTimeoutRef.current = setTimeout(() => {
      runVerificationAutoSave(form, step);
    }, 600);
  };
  const handleVerificationChange = (field, value) => {
    setVerificationForm((prev) => {
      ...
      queueVerificationAutoSave(next, latestStepRef.current);
      return next;
    });
  };
  ...
  const handleVerificationNext = () => {
    ...
    setVerificationStep((prev) => {
      const nextStep = Math.min(prev + 1, totalVerificationSteps - 1);
      latestStepRef.current = nextStep;
      queueVerificationAutoSave(latestFormRef.current, nextStep);
      return nextStep;
    });
  };
...
const buildVerificationAuditEntry = (message, status = 'pending') => ({
      ...
      if (verificationRequest?.id) {
        ...
        await updateDoc(verificationRef, {
          ...
          auditTrail: arrayUnion(
            buildVerificationAuditEntry('Pharmacy updated verification submission', 'pending')
          ),
        });
      } else {
        const newDoc = await addDoc(collection(db, 'verificationQueue'), {
  ...
  const canSubmitVerification =
    !verificationRequest ||
    ['rejected', 'needs_more_info', 'draft', ''].includes(verificationStatus);
  const verificationCtaLabel = verificationRequest
    ...
    <>
      ...
      >
        <form onSubmit={submitVerificationForm} className="flex flex-col gap-5">
          ...
          {isAutoSaving ? (
            <p className="text-[11px] text-slate-400 italic">Saving draft…</p>
          ) : verificationRequest?.status === 'draft' ? (
            <p className="text-[11px] text-slate-400">Draft saved</p>
          ) : null}
            ...
            <div className="flex gap-2">
              ...
              {verificationStep < totalVerificationSteps - 1 ? (
                ...
                <button
                  ...
                  className="rounded-full border border-sky-500 px-4 py-2 text-sm font-semibold text-sky-600 transition hover:bg-sky-50 focus:outline-none focus:ring-2 focus:ring-sky-200 disabled:cursor-not-allowed disabled:opacity-60 dark:border-sky-400 dark:text-sky-300 dark:hover:bg-slate-800"
                >
