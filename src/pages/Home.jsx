...
import useDebouncedLog from "../hooks/useDebouncedLog";
            ...
            <div className="flex justify-between items-center w-full h-[14px] md:h-[14px] lg:h-[20px]">
              <div className="flex flex-col justify-center">
                <div className="text-[17px] md:text-[26px] lg:text-[20px] font-regular font-poppins text-gray-900 dark:text-white">
                  {t("hello", "Hello")}
                  {user
                    ? `, ${user.displayName?.split(" ")[0] || t("friend", "Friend")}`
                    : ""}
                </div>
                <span
                  className="text-zinc-500 dark:text-zinc-400 text-[10px] md:text-[12px] lg:text-[14px] font-light font-poppins truncate max-w-xs md:max-w-md lg:max-w-lg"
                  title={location}
                >
                  {location}
                </span>
                ...
                <div className="flex flex-col justify-center items-end">
                  ...
                  <span className="text-[10px] md:text-[12px] lg:text-[14px] font-poppins font-thin text-right leading-tight mt-1.5 text-gray-800">
                    {profile?.role === "pharmacy"
                      ? t("your_pharmacy_location", "Your pharmacy location")
                      : t("location_services", "Location services")}
                  </span>
                </div>
              )}
            </div>
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    if (
      ...
      Array.isArray(serverResults.products)
    ) {
    ...
    return filteredByCategory.filter((p) => {
      ...
      const priceFields = [p.price, p.mrp, p.cost, p.listPrice]
      ...
      const matchesText =
        name.includes(ql) ||
        category.includes(ql) ||
        tags.includes(ql) ||
        vendorName.includes(ql) ||
        vendorLocation.includes(ql) ||
        priceFields.includes(ql);
      ...
      const matchesNumeric =
        numeric &&
        (priceFields.includes(numeric) ||
          (p.price && String(p.price).includes(numeric)));
  ...
  // Debounced logging for search (logs after 1.5s of typing)
  const searchPayload = useMemo(
    () => ({ term: q, resultsCount: pharmacyMatches.length }),
    [q, pharmacyMatches.length],
  );
  useDebouncedLog(searchPayload, 1500, logSearch);

  // useDebouncedLog(q, pharmacyMatches.length, 1500, logSearch);
  ...
  useEffect(() => {
    async function calculateETA() {
      ...
      const result = findClosestPharmacyWithETA(
        pharmacyList,
        userCoords,
        "driving",
      );
    ...
    const id = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/.netlify/functions/search?q=${encodeURIComponent(ql)}`,
          {
  ...
  const popularTotalPages = Math.max(
    1,
    Math.ceil(popularProducts.length / popularPerPage),
  );
  ...
  const normalizeHref = (link) => {
    ...
    if (/^(https?:)?\/\//i.test(l) || /^mailto:/i.test(l) || /^tel:/i.test(l))
      return l;
    // internal route
  ...
  useEffect(() => {
    ...
    const startUser = () => {
      ...
      }, 1800); // pause duration after user input
    };
    ...
    const tick = setInterval(() => {
      ...
      if (card && wrap) {
        ...
        const targetLeft = Math.max(
          0,
          Math.round(cardLeft - (wrap.clientWidth - card.clientWidth) / 2),
        );
        ...
        <div className="flex-1 overflow-y-auto w-full mx-auto flex flex-col pb-28 px-0">
          <div className="flex items-center gap-3 border-b border-zinc-300 dark:border-gray-600 dark:border-zinc-600 pb-2">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (!showSearchSuggestions || pharmacyMatches.length === 0)
                  return;
                ...
                switch (e.key) {
                  ...
                  case "Enter":
                    ...
                    if (
                      ...
                      selectedSuggestionIndex < pharmacyMatches.length
                    ) {
                    ...
                    if (q.trim().length >= 3) {
                      logSearch({
                        term: q.trim(),
                        resultsCount: pharmacyMatches.length,
                      });
                    }
                    break;
                  case "Escape":
            ...
            >
              <div className="flex gap-4 md:gap-6 min-w-max h-full">
                {infoCards.map((card, i) => {
                  ...
                  const previewColor =
                    card.previewColor ||
                    (card.headerColor ? card.headerColor + "cc" : "#ffffffcc");
                  const linkColor =
                    card.linkColor || card.linkTextColor || "#ffffff";
                  const headerFontSize = card.headerFontSize || 20; // px
                  const previewFontSize = card.previewFontSize || 13; // px
                  const linkFontSize =
                    card.linkTextFontSize || card.linkFontSize || 13;
