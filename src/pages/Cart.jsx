...
import { MapPin, Navigation, CreditCard, Truck, Shield, X, CheckCircle, Package, Clock, ChevronRight } from "lucide-react";
...
const OrderConfirmation = ({ orderDetails, onViewOrders, onContinueShopping }) => {
  ...
  const isPaid = orderDetails.paymentMethod === "online";

  return (
    <div className="fixed inset-0 z-[9999] bg-white dark:bg-gray-900 flex flex-col items-center justify-start overflow-y-auto">
      <div className={`w-full max-w-md mx-auto px-6 py-12 flex flex-col items-center transition-all duration-700 ${animateIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>

        {/* Success Icon */}
        <div className={`relative flex items-center justify-center w-24 h-24 rounded-full bg-green-50 dark:bg-green-900/20 mb-6 transition-all duration-700 delay-100 ${animateIn ? "scale-100 opacity-100" : "scale-50 opacity-0"}`}>
        ...
        {/* Title */}
        <div className={`text-center mb-2 transition-all duration-500 delay-200 ${animateIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          <h1 className="text-[26px] font-poppins font-light text-gray-900 dark:text-white">
            Order Placed!
          </h1>
          <p className="mt-2 text-[14px] text-gray-500 dark:text-gray-400 font-poppins font-light">
            {isPaid
        ...
        {/* Order ID */}
        <div className={`mt-4 px-4 py-2 bg-sky-50 dark:bg-sky-900/20 rounded-full transition-all duration-500 delay-300 ${animateIn ? "opacity-100" : "opacity-0"}`}>
          <span className="text-[12px] text-sky-600 dark:text-sky-400 font-poppins">
            Order #{orderDetails.orderId?.slice(0, 8).toUpperCase()}
          </span>
        </div>

        {/* Order Details Card */}
        <div className={`w-full mt-8 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden transition-all duration-500 delay-300 ${animateIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>

          {/* Items */}
          <div className="p-4 border-b border-gray-100 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-3">
              <Package className="w-4 h-4 text-gray-400" />
              <span className="text-[13px] font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wide">
                Items Ordered
              </span>
            </div>
            <div className="space-y-3">
              {orderDetails.items?.map((item, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  <div className="text-[13px] font-semibold text-sky-600 dark:text-sky-400 flex-shrink-0">
          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`text-[12px] px-3 py-1 rounded-full font-medium ${isPaid ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"}`}>
                {isPaid ? "Paid" : "Pay on Delivery"}
              </span>
            </div>

            {orderDetails.deliveryFee > 0 && (
              <div className="flex justify-between items-center text-[13px] text-gray-500 dark:text-gray-400 mb-1">
                <span>Delivery Fee</span>
                <span>₦{Number(orderDetails.deliveryFee).toLocaleString()}</span>
              </div>
            )}
            <div className="flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-700 mt-2">
              <span className="text-[15px] font-semibold text-gray-900 dark:text-white">Total</span>
              <span className="text-[17px] font-bold text-sky-600 dark:text-sky-400">
        ...
        {/* ETA Banner */}
        <div className={`w-full mt-4 p-4 bg-sky-50 dark:bg-sky-900/20 rounded-2xl flex items-center gap-3 transition-all duration-500 delay-400 ${animateIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          <Clock className="w-5 h-5 text-sky-500 flex-shrink-0" />
          <div>
            <p className="text-[13px] font-medium text-sky-700 dark:text-sky-400">
              Estimated Delivery
            </p>
            <p className="text-[12px] text-sky-500 dark:text-sky-500">
        ...
        {/* Action Buttons */}
        <div className={`w-full mt-8 flex flex-col gap-3 transition-all duration-500 delay-500 ${animateIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          <button
            onClick={onViewOrders}
            className="w-full py-3 rounded-full bg-sky-600 text-white text-[15px] font-poppins font-medium hover:bg-sky-700 active:scale-95 transition-all duration-200 flex items-center justify-center gap-2"
          >
            View My Orders
          ...
          <button
            ...
            className="w-full py-3 rounded-full border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 text-[15px] font-poppins font-light hover:border-sky-400 hover:text-sky-600 active:scale-95 transition-all duration-200"
          >
            Continue Shopping
          </button>
        </div>
...
export default function Cart() {
  ...
  const startCheckout = () => setShowOrderSummary(true);
  ...
  const handleFinalCheckout = async () => {
    ...
    try {
      ...
      // Show confirmation page instead of toast + navigate
      setConfirmedOrder({
        orderId: typeof orderStatus === "string" ? orderStatus : Date.now().toString(),
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1">
                <button
                  ...
                  onClick={async () => {
                    if (i.qty > 1) await removeFromCart(user.uid, i.ids[0]);
                  }}
                  disabled={i.qty <= 1}
            ...
            <div className="p-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg text-[14px] pr-10 dark:bg-gray-700 dark:text-white"
                      />
                      <button
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg text-[14px] dark:bg-gray-700 dark:text-white"
                      />
                    </div>
                    <div>
                      <label className="block text-[14px] font-medium text-gray-700 dark:text-gray-300 mb-2">
              ...
              <div className="mb-6 p-4 bg-sky-50 dark:bg-gray-700 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[14px] text-gray-700 dark:text-gray-300">{t("subtotal", "Subtotal")}</span>
                  <span className="text-[14px] font-medium text-gray-900 dark:text-white">₦{Number(total).toLocaleString()}</span>
                </div>
                {deliveryFee > 0 && (
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-[14px] text-gray-700 dark:text-gray-300">{t("delivery_fee", "Delivery Fee")}</span>
                    <span className="text-[14px] text-gray-900 dark:text-white">₦{Number(deliveryFee).toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between items-center pt-2 border-t border-sky-200 dark:border-gray-600">
                  <span className="text-[16px] font-medium text-gray-900 dark:text-white">{t("total", "Total")}</span>
                  <span className="text-[16px] font-bold text-sky-600 dark:text-sky-400">₦{Number(totalWithDelivery).toLocaleString()}</span>
                </div>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[16px] font-medium dark:text-white">{t("pay_on_delivery", "Pay on Delivery")}</div>
                      <div className="text-[13px] text-gray-600 dark:text-gray-300">{t("pay_with_cash_on_delivery", "Pay with cash when order arrives")}</div>
                    </div>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[16px] font-medium dark:text-white">{t("online_payment", "Online Payment")}</div>
                      <div className="text-[13px] text-gray-600 dark:text-gray-300">{t("pay_with_card_bank_ussd", "Pay with card, bank transfer, or USSD")}</div>
                    </div>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-3 justify-between">
                    ...
                    <span className="text-[12px] text-gray-500 bg-gray-200 px-2 py-1 rounded-full">{t("coming_soon", "Coming Soon")}</span>
                  </div>
                </button>
              </div>
              ...
              <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="flex justify-between items-center">
                  <span className="text-[16px] font-medium dark:text-white">{t("total_amount", "Total Amount")}</span>
