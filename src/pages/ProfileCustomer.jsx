...
import ProfileCustomerSkeletonMobile from '@/components/ProfileCustomerSkeletonMobile';
import ProfileCustomerSkeletonDesktop from '@/components/ProfileCustomerSkeletonDesktop';
...
export default function ProfileCustomer() {
  ...
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      setLoading(true);
      try {
        const [cartSnap, chatsSnap, ordersSnap] = await Promise.all([
          getDocs(collection(db, 'users', user.uid, 'cart')),
          getDocs(query(collection(db, 'threads'), where('customerId', '==', user.uid))),
        ...
        for (const docSnap of ordersSnap.docs) {
          ...
          for (const item of order.items || []) {
            ...
            if (!drugs[key]) {
              ...
              drugs[key] = { id: key, name, count: 0, date: orderDate };
            }
        ...
        const sortedDrugs = Object.values(drugs).sort(
          (a, b) => (b.date?.getTime?.() || 0) - (a.date?.getTime?.() || 0)
        );

        if (isMounted) {
          setDrugsBought(sortedDrugs);
        }
      } catch (error) {
    ...
    const unsubscribeWishlist = listenUserWishlist(user.uid, (wishlistItems) => {
      if (isMounted) setWishlist(wishlistItems);
    });

    const unsubscribeBookmarks = listenUserBookmarkedVendors(user.uid, (bookmarks) => {
      if (isMounted) setBookmarkedVendors(bookmarks);
    });
    ...
    return (
      <div className="px-4 md:px-6 lg:px-12 py-8">
        <div className="md:hidden">
          <ProfileCustomerSkeletonMobile />
        </div>
        <div className="hidden md:block">
          <ProfileCustomerSkeletonDesktop />
        </div>
      </div>
    );
  }
