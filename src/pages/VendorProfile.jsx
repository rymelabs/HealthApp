...
import { listenProducts, getOrCreateChatThread } from '@/lib/db';
...
export default function VendorProfile() {
  ...
  const handleMessageVendor = async () => {
    if (!user || !user.uid) {
      alert('You must be signed in to message vendors.');
      return;
    }
    try {
  ...
  return (
    <div className="min-h-screen bg-white/80 backdrop-blur-md max-w-md md:max-w-2xl lg:max-w-4xl xl:max-w-6xl mx-auto px-5 md:px-8 lg:px-12 xl:px-0 pt-8 pb-28">
      ...
      <div className="border border-zinc-200 rounded-2xl bg-white shadow-sm p-5 mb-6 w-full flex flex-col items-start">
        ...
        <div className="flex items-center gap-2 text-zinc-500 text-[13px] font-poppins font-light mb-1"><MapPin className="h-3 w-3"/> {vendor.address}</div>
        <div className="flex items-center gap-2 text-zinc-500 text-[13px] font-poppins font-light mb-1"><Clock className="h-3 w-3"/> {vendor.etaMins || 25} mins to {vendor.name}</div>
        <div className="flex items-center gap-2 text-zinc-500 text-[13px] font-poppins font-light"><Phone className="h-3 w-3"/> {vendor.phone}</div>
      </div>
      <button
      ...
      <div className="space-y-3">
        {(showAll ? products : products.slice(0, 3)).map(p => (
          <div key={p.id} className="rounded-2xl border border-zinc-200 p-3 flex items-center gap-3 bg-white shadow-sm cursor-pointer hover:bg-sky-50 transition"
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-zinc-500 text-[12px] font-poppins font-light">{p.category} • Stock: {p.stock} • SKU: {p.sku}</div>
            </div>
            <div className="text-[15px] font-poppins font-medium text-sky-600">₦{Number(p.price).toLocaleString()}</div>
          </div>
        ))}
        {products.length===0 && <div className="text-zinc-500 text-[13px] font-poppins font-light">No products yet.</div>}
      </div>
      {products.length > 3 && showAll && (
