              ...
              >
                ...
                <div className="md:col-span-2">
                  {(() => {
                    ...
                    const label = STATUS_LABELS[statusKey] || statusKey || "Pending";
                    const tone =
                      STATUS_TONES[statusKey] ||
                      "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300";
                    return (
                      <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium capitalize ${tone}`}>
                        {label}
                      </span>
                    );
                  })()}
                ...
                <div className="flex items-center justify-end gap-2 md:col-span-3">
                  <button
                    type="button"
                    onClick={() => { setSelected(item); setNote(item.reviewerNote || ""); }}
        ...
        {selected && (
          <div className="flex flex-col gap-6">
            <header className="space-y-2">
              ...
              {(() => {
                const statusKey = (selected.status || "pending").toLowerCase();
                const label = STATUS_LABELS[statusKey] || statusKey || "Pending";
                const tone =
                  STATUS_TONES[statusKey] ||
                  "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300";
                return (
                  <div className="flex flex-wrap items-center gap-3">
                    <span
                      className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide ${tone}`}
                    >
                      {label}
                    </span>
