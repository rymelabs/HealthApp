  ...
  return (
    <>
      ...
      <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-300 ${
        ...
        <div className="bg-white rounded-2xl shadow-2xl border border-blue-200 mx-4 overflow-hidden" style={{ width: '393px' }}>
          ...
          <div className="bg-gradient-to-r from-sky-300 to-sky-600 px-6 py-4 relative overflow-hidden">
            ...
            <div className="relative z-10 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-white font-semibold text-lg">New Order Received!</h3>
                <p className="text-green-100 text-sm">You have a new customer order</p>
              </div>
            </div>
          </div>
          ...
          <div className="px-4 py-3">
            <div className="space-y-2">
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="font-bold text-green-600 text-sm">₦{(order.total || 0).toLocaleString()}</p>
                </div>
              </div>
                ...
                <div className="border-t pt-3">
                  ...
                  <div className="max-h-24 overflow-y-auto space-y-1">
                    {(order.items || order.orderItems || order.products || []).slice(0, 3).map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center text-xs gap-2 py-0.5">
                        ...
                        <span className="text-gray-600 text-right whitespace-nowrap font-semibold bg-gray-100 px-1.5 py-0.5 rounded text-xs flex-shrink-0">
                          x{item.quantity || item.qty || item.amount || 1}
                        </span>
                      </div>
                    ))}
                    {(order.items?.length || order.orderItems?.length || order.products?.length || 0) > 3 && (
                      <p className="text-xs text-gray-500 italic mt-1 text-center bg-gray-50 py-0.5 rounded">
              ...
              <div className="border-t pt-2 flex justify-between items-center">
                <div>
                  ...
                  <span className="inline-block px-2 py-0.5 bg-orange-100 text-orange-700 text-xs rounded-full font-medium mt-0.5">
            ...
            <div className="flex gap-2 mt-4 pt-3 border-t">
              <button
                ...
                className="flex-1 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium text-xs transition-all duration-200 shadow-lg hover:shadow-xl"
              >
                View Details
              </button>
              <button
                onClick={handleClose}
                className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-medium text-xs transition-all duration-200"
              >
                Dismiss
              </button>
            </div>
          ...
          <button
            onClick={handleClose}
            className="absolute top-3 right-3 w-8 h-8 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center text-white transition-colors duration-200"
          >
