...
export const getOrCreateThread = async ({ vendorId, customerId, role }) => {
  ...
  if (!snap.exists()) {
    if (role !== "customer")
      throw new Error("Only customers can start a chat thread");
...
export const getBestSellingProducts = async (limitCount = 5, pharmacyId) => {
  ...
  const productsSnap = await getDocs(
    query(collection(db, "products"), where("pharmacyId", "==", pharmacyId))
  );
...
export async function fulfillPrescriptionIfOrdered({ customerId }) {
  ...
  const presSnap = await getDocs(
    query(
      ...
      where("requirePurchase", "==", true),
      where("fulfilled", "!=", true)
    )
  );
  const prescriptions = presSnap.docs.map((doc) => ({
    id: doc.id,
    ...doc.data(),
  }));
  ...
  for (const p of prescriptions) {
    const allDrugsPurchased = p.drugs.every(
      (d) => d.productId && allOrderItems.includes(d.productId)
    );
